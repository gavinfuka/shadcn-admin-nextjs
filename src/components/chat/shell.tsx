...
export function ChatShell() {
  ...
  const handleEditMessage = useCallback((message: ChatMessage) => {
    ...
    chat.setInput(message.parts.filter((part) => part.type === 'text').map((part) => part.text).join(''))
  }, [chat])
  ...
  const handleSendEditedMessage = useCallback(async () => {
    ...
    await chat.sendMessage({ parts: [{ text: chat.input, type: 'text' }], role: 'user' })
    chat.setInput('')
  }, [chat, editingMessage])
  ...
  return (
    <main className='flex h-svh min-h-0 w-full overflow-hidden bg-background'>
      <section className={artifact.isVisible ? 'flex min-w-0 flex-1 flex-col' : 'flex w-full min-w-0 flex-col'}>
        ...
        <div className='mx-auto w-full max-w-4xl'>
          <MultimodalInput
            ...
            isLoading={chat.isLoading}
            messages={chat.messages}
            onCancelEdit={() => { setEditingMessage(null); chat.setInput('') }}
      ...
      <Artifact addToolApprovalResponse={chat.addToolApprovalResponse} attachments={attachments} chatId={chat.chatId} input={chat.input} isReadonly={chat.isReadonly} messages={chat.messages} regenerate={chat.regenerate} selectedModelId={chat.currentModelId} selectedVisibilityType={chat.visibilityType} sendMessage={chat.sendMessage} setAttachments={setAttachments} setInput={chat.setInput} setMessages={chat.setMessages} status={chat.status} stop={chat.stop} votes={chat.votes} />
      <DataStreamHandler />
    </main>
  )
}