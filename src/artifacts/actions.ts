...
export async function saveArtifactContent({ id, title, kind, content }: { id: string; title: string; kind: ArtifactKind; content: string }) {
  return localChatPersistence.saveDocument({ id, title, kind, content, createdAt: new Date() })
}
